...
(async()=>{
 ...
 for(const lang of ['vi','en']){
  ...
  assert(await v.evaluate(n=>n.duration>90&&n.duration<120&&n.videoWidth===1280&&n.videoHeight===720));
  assert.equal(await v.locator('track').getAttribute('srclang'),lang);
  const chapters=p.locator('[data-story-seek]');assert.equal(await chapters.count(),6);
 ...
 assert.deepEqual(errors,[]);await b.close();console.log('PASS: both story films, audio/video decoding, 6 chapter seeks, captions, transcript, 3 widths, deterministic Canvas seeking and narrated offline HTML.');
