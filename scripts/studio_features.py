...
def render_story(language):
 ...
 return f'''<section class="container studio-story" id="story"><div class="studio-section-heading"><h2>{'Why AI Studio?' if en else 'Vì sao có AI Studio?'}</h2><p>{'From scattered knowledge to a connected AI workflow. A drawn story, narrated in English.' if en else 'Từ tri thức rời rạc đến một luồng công việc AI kết nối. Câu chuyện bằng nét vẽ và lời dẫn tiếng Việt.'}</p></div><video id="studio-story" controls preload="metadata" playsinline poster="{base}poster.{language}.png" aria-label="{'Why AI Studio — drawn story' if en else 'Vì sao có AI Studio — câu chuyện bằng nét vẽ'}"><source src="{base}story.{language}.mp4" type="video/mp4"><track kind="captions" src="{base}captions.{language}.vtt" srclang="{language}" label="{'English' if en else 'Tiếng Việt'}" default></video><div class="story-chapters" role="group" aria-label="{'Story chapters' if en else 'Các phần của câu chuyện'}">{chapters}</div><p class="story-meta">{duration} · {'Drawn animation · Synthetic narration' if en else 'Hoạt hình nét vẽ · Giọng đọc tổng hợp'} · <a href="{base}story.{language}.mp4" download>{'Download video' if en else 'Tải video'}</a></p><details class="story-transcript"><summary>{'Read the story' if en else 'Đọc lời dẫn'}</summary>{transcript}</details></section>'''
