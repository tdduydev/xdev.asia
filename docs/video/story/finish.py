...
ROOT=Path(__file__).resolve().parents[3];OUT=ROOT/'src/assets/ai-studio/story';WORK=Path('/tmp/studio-story');RENDER=Path('/tmp/story-render');FF=imageio_ffmpeg.get_ffmpeg_exe()
...
for lang in ['vi','en']:
 ...
 run(['-f','concat','-safe',0,'-i',concat,'-c:a','aac','-b:a','128k',OUT/f'narration.{lang}.m4a'])
 run(['-i',RENDER/f'story.{lang}.mp4','-i',OUT/f'narration.{lang}.m4a','-map','0:v:0','-map','1:a:0','-c','copy','-movflags','+faststart','-t',d['duration'],OUT/f'story.{lang}.mp4'])
 run(['-ss',d['scenes'][-1]['start']+9,'-i',OUT/f'story.{lang}.mp4','-frames:v',1,OUT/f'poster.{lang}.png'])
